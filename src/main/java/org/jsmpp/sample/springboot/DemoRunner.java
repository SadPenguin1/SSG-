...
import org.jsmpp.sample.springboot.jsmpp.SmppClientService2;
...
public class DemoRunner implements ApplicationRunner {
  ...
  @Autowired
  private SmppClientService2 smppClientService2;
  ...
  public void run(final ApplicationArguments applicationArguments) throws Exception {
	...
	System.err.println("Start send : " + startTime);
	
    demoTime();
  //  System.err.println("End send : " + (System.currentTimeMillis() - startTime));
  }

  public void demoTime() throws InterruptedException {
	// Bắt đầu lắng nghe 
    smppServerService.start();
    System.err.println("Start listen ...");
//    final List<Future<Long>> futureList = new ArrayList<>();
//    for (int i = 0; i < config.getNumberOfClientSessions(); i++) {
//      // Start client sessions asynchronously with random number of messages ( đây là chỗ bắt đầu gửi tin nè )
...
//      Thread.sleep(1000);
//    }
    //  tiếp tục gửi sau khi vừa gửi và nhận
//	futureList.add(smppClientService2.start(RandomUtils.nextInt(config.getMinMessagesPerSession(), config.getMaxMessagesPerSession())));
